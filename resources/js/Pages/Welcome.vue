...
<template>
<Head title="SDB Bank — Coming Soon">
  <meta name="description" content="SDB Bank — The new digital bank. Multi-currency accounts, Mastercard cards, crypto trading." />
...
<section class="hero">
  <div class="mx hero-mx">
    <div class="htag a">LAUNCHING SOON</div>
    <h1 class="hd a">
      <span class="hd-line">Banking for</span>
      <span class="hd-em">a new era</span>
    </h1>
    <p class="hp a">Multi-currency accounts · Instant Mastercard · Crypto with real wallets · Transfers to 150+ countries. All in one place.</p>

    <div class="cd a">
      <div v-for="(lb,k) in {d:'Days',h:'Hrs',m:'Min',s:'Sec'}" :key="k" class="cd-b">
    ...
    <div class="eml a">
      <template v-if="!done">
        <input v-model="em" type="email" placeholder="Your email..." class="eml-i" @keyup.enter="done=!!em"/>
        <button @click="done=!!em" class="eml-b">Notify me</button>
      </template>
      <div v-else class="eml-ok">✓ Done! We'll let you know at launch.</div>
    </div>

    <div class="trust a">
      <span class="trust-i">🇩🇰 Licensed in Denmark</span>
      <span class="trust-i">🔒 256-bit encrypted</span>
      <span class="trust-i">🏦 Mastercard certified</span>
    </div>
  </div>
</section>
...
<section class="sec sec-w">
  <div class="mx">
    <div class="sec-hdr a">
      <h2 class="t2">Everything you need.<br><span class="t2-fade">Nothing you don't.</span></h2>
    </div>
    <div class="fg">
      <div v-for="(f,i) in [
        {ic:'💳',t:'Mastercard Cards',p:'Instant virtual card on signup. Or order a premium metal card. Pay with Apple Pay anywhere in the world.',tag:'CARDS'},
        {ic:'💱',t:'30+ Global Currencies',p:'Hold and convert at real market rates. No hidden fees. EUR, USD, GBP, AED, SAR and more.',tag:'FX'},
        {ic:'🪙',t:'Crypto with Real Wallets',p:'Buy and sell BTC, ETH, SOL and more. Every customer gets a real, globally recognized wallet address. Send and receive freely.',tag:'CRYPTO'},
        {ic:'⚡',t:'International Transfers',p:'Send money to 150+ countries via SWIFT & SEPA. Arrives in minutes with the lowest fees possible.',tag:'TRANSFERS'},
        {ic:'🛡️',t:'Unbreakable Security',p:'Bank-grade 256-bit encryption. Two-factor authentication and Face ID. 24/7 fraud monitoring team.',tag:'SECURITY'},
        {ic:'📱',t:'One App for Everything',p:'iOS & Android. Full control of your accounts, cards, trades and wallets — all from one place.',tag:'APP'}
...
<section class="sec sec-bk">
  <div class="mx">
    <div class="sec-hdr a">
      <h2 class="t2 text-white">Your crypto.<br><span class="t2-fade-w">Your real wallet.</span></h2>
      <p class="sp sp-w">Buy, sell, send and receive — blockchain wallets in your name.</p>
    </div>
    ...
    <div class="cr-grid a">
      <div class="cr-box"><span class="cr-box-ic">📥</span><span class="cr-box-t">Receive</span><span class="cr-box-d">Your unique wallet address</span></div>
      <div class="cr-box"><span class="cr-box-ic">📤</span><span class="cr-box-t">Send</span><span class="cr-box-d">To any wallet worldwide</span></div>
      <div class="cr-box"><span class="cr-box-ic">🔄</span><span class="cr-box-t">Swap</span><span class="cr-box-d">Between crypto & fiat</span></div>
      <div class="cr-box"><span class="cr-box-ic">📊</span><span class="cr-box-t">Track</span><span class="cr-box-d">Live prices & portfolio</span></div>
    </div>
  </div>
</section>
...
<section class="sec sec-w">
  <div class="mx">
    <div class="sec-hdr a">
      <h2 class="t2">Clear plans.<br><span class="t2-fade">No surprises.</span></h2>
    </div>
    <div class="pl a">
      <div v-for="(p,i) in [
        {n:'Standard',pr:'Free',ft:['Virtual Mastercard','EUR + USD + GBP accounts','Apple Pay & Google Pay','Free SEPA transfers']},
        {n:'Premium',pr:'7.99€',pop:true,ft:['All Standard features','Unlimited FX','Full travel insurance','3 lounge visits/mo']},
        {n:'Elite',pr:'14.99€',ft:['All Premium features','Unlimited VIP lounges','1% cashback','Personal account manager']}
      ]" :key="i" class="pl-c" :class="{'pl-pop':p.pop}">
        <div v-if="p.pop" class="pl-badge">POPULAR</div>
        <div class="pl-n">{{ p.n }}</div>
        <div class="pl-pr">{{ p.pr }}<span v-if="p.pr!=='Free'" class="pl-mo">/mo</span></div>
        <div class="pl-div"></div>
        <ul class="pl-ul"><li v-for="f in p.ft" :key="f"><span class="pl-ck" :class="{'pl-ck-w':p.pop}">✓</span>{{ f }}</li></ul>
        <Link href="/preregister" class="pl-btn" :class="{'pl-btn-w':p.pop}">Get started</Link>
      </div>
    </div>
  </div>
</section>
...
<section class="sec sec-bk">
  <div class="mx">
    <div class="st a">
      <div v-for="s in [{v:'30+',l:'Currencies'},{v:'6+',l:'Cryptos'},{v:'150+',l:'Countries'},{v:'24/7',l:'Support'}]" :key="s.v" class="st-i">
...
<section class="sec sec-w" style="padding:120px 0">
  <div class="mx text-center">
    <h2 class="t2 a">Ready for the future?</h2>
    <p class="sp a" style="margin-bottom:32px">Sign up now and get early access. Completely free.</p>
    <Link href="/preregister" class="cta-big a">Open your free account →</Link>
  </div>
</section>
  ...
  <div class="mx">
    <div class="ft-g">
      <div>
        ...
        <p class="ft-d">Licensed digital bank in Denmark.
Banking with European standards.</p>
      </div>
      <div>
        <div class="ft-h">Products</div>
        <div class="ft-lk"><Link href="/currencies">Currencies</Link><Link href="/cards-info">Cards</Link><Link href="/transfers-info">Transfers</Link></div>
      </div>
      <div>
        <div class="ft-h">Legal</div>
        <div class="ft-lk"><Link href="/terms">Terms</Link><Link href="/privacy">Privacy</Link><Link href="/about">About</Link></div>
      </div>
      <div>
    ...
    <div class="ft-btm">
      <span>© 2026 SDB Bank ApS. All rights reserved.</span>
    </div>
  </div>
</footer>
...
.sp{font-size:16px;opacity:.35;line-height:1.8;max-width:440px;margin:0 auto 48px}.sp-w{color:#fff}
...
.pl-badge{position:absolute;top:-11px;left:20px;font-size:10px;font-weight:800;letter-spacing:1.5px;background:#2563EB;color:#fff;padding:4px 14px;border-radius:6px;text-transform:uppercase}
