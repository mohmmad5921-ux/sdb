...
class _ExchangeScreenState extends State<ExchangeScreen> {
  ...
  Future<void> _submit() async {
    if (fromAccount == null || toAccount == null || _amount.text.isEmpty) {
      setState(() => error = 'يرجى تحديد الحسابات والمبلغ'); return;
    }
  ...
  void _swap() {
    if (fromAccount != null && toAccount != null) {
      setState(() { final t = fromAccount; fromAccount = toAccount; toAccount = t; });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('صرف عملات', style: TextStyle(fontWeight: FontWeight.w700)), backgroundColor: Colors.transparent, elevation: 0,
        ...
        : SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            Center(child: GestureDetector(
              ...
              child: Container(width: 44, height: 44,
                decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0xFFF59E0B).withValues(alpha: 0.1), border: Border.all(color: const Color(0xFFF59E0B).withValues(alpha: 0.2))),
            ...
            // Amount
            Padding(padding: const EdgeInsets.only(bottom: 8), child: Text('المبلغ', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.white.withValues(alpha: 0.5)))),
            Container(
              ...
              child: TextField(controller: _amount, keyboardType: const TextInputType.numberWithOptions(decimal: true), textDirection: TextDirection.ltr,
                ...
                decoration: InputDecoration(hintText: '0.00', hintStyle: TextStyle(color: Colors.white.withValues(alpha: 0.1), fontSize: 24, fontWeight: FontWeight.w700),
                  border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(vertical: 20),
                  suffixText: fromAccount?['currency']?['code'] ?? '', suffixStyle: TextStyle(color: Colors.white.withValues(alpha: 0.3), fontSize: 14))),
            ),
  ...
  Widget _accountBox(String label, Map<String, dynamic>? account, ValueChanged<int?> onChanged) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label, style: TextStyle(fontSize: 11, color: Colors.white.withValues(alpha: 0.3))),
        const SizedBox(height: 8),
        DropdownButton<int>(isExpanded: true, underline: const SizedBox(), dropdownColor: const Color(0xFF1A1A2E),
          value: account?['id'], style: const TextStyle(color: Colors.white, fontSize: 14),
          items: accounts.map<DropdownMenuItem<int>>((a) => DropdownMenuItem(value: a['id'] as int,
            child: Row(children: [
              Container(width: 32, height: 32, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withValues(alpha: 0.1)),
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('${a['currency']?['code'] ?? ''}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.white)),
                Text('${fmt(a['balance'])}', style: TextStyle(fontSize: 11, color: Colors.white.withValues(alpha: 0.4))),
              ])),
            ]))).toList(),
          onChanged: onChanged),
      ]),
