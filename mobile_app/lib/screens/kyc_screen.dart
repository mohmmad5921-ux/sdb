...
class _KycScreenState extends State<KycScreen> {
  ...
  Widget _buildDocTypeSelection() => SingleChildScrollView(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
    Container(padding: const EdgeInsets.all(20),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 12),
        const Text('مسح المستندات', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: Colors.white)),
    ...
    const Text('اختر نوع المستند', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: AppTheme.textPrimary)),
    ...
    const SizedBox(height: 24),
    _btn(_docType != null ? 'بدء المسح 📷' : 'اختر نوع المستند', _docType != null ? () => setState(() => _step = 1) : null),
  ]));
        ...
        child: file != null
          ? Stack(children: [
              ...
              Positioned(top: 8, right: 8, child: Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                ...
                child: const Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.check, color: Colors.white, size: 14), SizedBox(width: 4), Text('تم المسح', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w600))]))),
            ])
          : Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              const SizedBox(height: 12),
              const Text('اضغط لفتح السكانر', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppTheme.textMuted)),
            ]),
      )),
      ...
      if (file != null) ...[
        ...
        Center(child: GestureDetector(onTap: () => _captureDocument(isFront),
          child: const Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.refresh_rounded, size: 16, color: AppTheme.primary), SizedBox(width: 4),
            Text('إعادة المسح', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppTheme.primary))]))),
      ],
  ...
  Widget _buildNameVerification() => Padding(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
    _progress(3, 4),
    const SizedBox(height: 24),
    const Text('التحقق من الاسم', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: AppTheme.textPrimary)),
    const SizedBox(height: 20),
    Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: AppTheme.bgCard, borderRadius: BorderRadius.circular(16), border: Border.all(color: AppTheme.border)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('الاسم المسجل:', style: TextStyle(fontSize: 12, color: AppTheme.textMuted)),
        const SizedBox(height: 4),
        Text(_userName ?? 'غير متوفر', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: AppTheme.textPrimary)),
        const SizedBox(height: 16),
        const Text('الاسم المستخرج من المستند:', style: TextStyle(fontSize: 12, color: AppTheme.textMuted)),
        const SizedBox(height: 4),
  ...
  Widget _buildSelfieStep() => Padding(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
    _progress(4, 4),
    const SizedBox(height: 24),
    const Text('صورة سيلفي', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: AppTheme.textPrimary)),
    const SizedBox(height: 6),
      ...
      child: _selfie != null
        ? Stack(children: [
            ...
            if (_faceDetected) Positioned(top: 8, right: 8, child: Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
              decoration: BoxDecoration(color: AppTheme.success, borderRadius: BorderRadius.circular(12)),
              child: const Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.face, color: Colors.white, size: 14), SizedBox(width: 4), Text('وجه مكتشف', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w600))]))),
          ])
        : Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            const SizedBox(height: 12),
            const Text('اضغط لفتح الكاميرا', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppTheme.textMuted)),
          ]),
    )),
    ...
    if (_selfie != null) ...[
      ...
      Center(child: GestureDetector(onTap: _captureSelfie,
        child: const Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.refresh_rounded, size: 16, color: AppTheme.primary), SizedBox(width: 4),
          Text('إعادة التصوير', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppTheme.primary))]))),
    ],
    if (_error != null) ...[const SizedBox(height: 12), _errorBox(_error!)],
    const Spacer(),
