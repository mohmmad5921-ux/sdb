...
class MoreTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SafeArea(child: ListView(padding: const EdgeInsets.all(20), children: [
        // Header
        const Text('الإعدادات', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w800, color: Colors.white)),
        ...
        Container(
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('إدارة معلوماتك وإعداداتك', style: TextStyle(fontSize: 12, color: Colors.white.withValues(alpha: 0.35))),
            ])),
            Container(
              width: 36, height: 36,
              decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(10)),
              child: Icon(Icons.arrow_forward_ios_rounded, size: 14, color: Colors.white.withValues(alpha: 0.3)),
            ),
          ]),
        ),
        ...
        Center(child: Text('SDB Banking v1.0.0', style: TextStyle(fontSize: 11, color: Colors.white.withValues(alpha: 0.1)))),
  ...
  void _logout(BuildContext context) async {
    final ok = await showDialog<bool>(context: context, builder: (_) => AlertDialog(
      ...
      title: const Text('تسجيل الخروج', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 18)),
      content: Text('هل تريد تسجيل الخروج من حسابك؟', style: TextStyle(color: Colors.white.withValues(alpha: 0.5), fontSize: 14)),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context, false), child: Text('إلغاء', style: TextStyle(color: Colors.white.withValues(alpha: 0.4)))),
  ...
  Widget _section(String t) => Padding(padding: const EdgeInsets.only(bottom: 10, right: 4), child: Text(t, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.white.withValues(alpha: 0.25), letterSpacing: 0.5)));
    ...
    return GestureDetector(
      ...
      child: Padding(
        ...
        child: Row(children: [
          ...
          Icon(Icons.arrow_forward_ios_rounded, size: 14, color: Colors.white.withValues(alpha: 0.15)),
        ]),
      ),
    );
  }
